...
Next, write the <code>board.on</code> function. Notice that you are definine two Johnny Five objects and injecting them into the REPL.
<pre>
...
While in the loop, set the _ligthOn_ value (true/false) based on a predefined ambientLight threshold (you should use the smae threshold you used in Lab 3), and clode the <code>forEach</code> loop.
<pre>        
...
Now that the <code>executeQueue()</code> function has set the state of the LED you need to compose a response (downstream) message to send. The response is a <code>\_lightState</code> message. The <code>command</code> in this message is simply the current <code>on</code> state of the LED (on: true/false). Notice that there is a <code>response\_to</code> value in the message that contains all of the command IDs that this downstream message is in response to. This helps with the <code>obsoletes()</code> function as well as providing any message subscribers the information to correlate the response to one or more commands it is in response to.
<pre>
...
In the [next lab][9] you will learn how to visualize the data coming from your connected Thing in a web site or a mobile app.
